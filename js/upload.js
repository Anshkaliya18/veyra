...
const totalDocumentsEl = document.querySelector("[data-upload-total]");
...
async function watchUploadTask(taskId, row) {
  while (true) {
    try {
      ...
      if (data.status === "failed") {
        setRowStatus(row, {
          badge: "Failed",
          status: data.error ? `✗ ${data.error}` : "✗ Failed",
          progress: "100%",
        });
        break;
      }
      ...
      console.error("Status watch failed:", err);
      setRowStatus(row, {
        badge: "Failed",
        status: "✗ Failed",
        progress: "100%",
  ...
  for (let index = 0; index < files.length; index++) {
    ...
    try {
      ...
      if (!data.task_id) {
        setRowStatus(row, {
          badge: "Failed",
          status: "Missing task ID",
          progress: "100%",
        });
        continue;
      }

      await watchUploadTask(data.task_id, row);
    } catch (err) {
...
async function loadFiles() {
  try {
    ...
    if (totalDocumentsEl) {
      totalDocumentsEl.textContent = String(files.length);
    }
...
$("#clear-files")?.addEventListener("click", async () => {
  try {
    const response = await fetch("/api/uploads/clear-completed", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    if (!response.ok || !data.success) {
      alert(data.message || "Unable to clear completed uploads.");
      return;
    }
    ...
    alert("Unable to clear completed uploads.");
  }
});
