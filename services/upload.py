...
def update_status(status_callback, stage, progress):
    if status_callback:
        status_callback(
            stage=stage,
            progress=progress
...
def _mark_upload_completed(file_id: int) -> None:
    conn = get_db_connection()
    cur = None
    ...
    try:
        ...
        conn.rollback()
        logger.exception("Failed to mark upload as completed")
...
def upload_file(
    ...
    update_status(status_callback, "Uploading", 10)

    uploaded = _upload_to_supabase(file, user_id)
    update_status(status_callback, "Uploaded", 25)

    file_id = None
    try:
        update_status(status_callback, "Saving metadata", 30)
        ...
        try:
            ...
            # Extract text
            update_status(status_callback, "Extracting", 50)
            result = extract_text(
                temp_file.name,
            ...
            if result["success"]:
                ...
                update_status(status_callback, "Extracted", 65)

                update_status(status_callback, "Summarizing", 75)
                ai_result = summarize_document(
                    text,
                    status_callback=status_callback
                )
                ...
                if ai_result["success"]:
                    ...
                    update_status(status_callback, "Summary saved", 90)
                else:
                    logger.warning(
                ...
                raise UploadError(f"Text extraction failed: {result['error']}")
        ...
        if file_id is not None:
            _mark_upload_completed(file_id)
        ...
        update_status(status_callback, "Failed", 100)
...
def get_upload_overview(user_id: int) -> dict[str, Any]:
    uploads = get_user_uploads(user_id)
    total_documents = len(uploads)
    completed_documents = sum(1 for item in uploads if str(item.get("upload_status") or "").lower() == "completed")
    failed_documents = sum(1 for item in uploads if str(item.get("upload_status") or "").lower() == "failed")
    active_documents = max(total_documents - completed_documents - failed_documents, 0)
    ...
    return {
        ...
        "progress_pct": min(100, 20 + total_documents * 7 + completed_documents * 5),
    }


def clear_completed_uploads(user_id: int) -> int:
    conn = get_db_connection()
    cur = None
    ...
    try:
        ...
        cur.execute(
            ...
            FROM uploaded_files
            WHERE user_id = %s AND LOWER(upload_status) = 'completed'
            """,
            (user_id,),
        )
        ...
        for row in rows:
            if hasattr(row, "keys"):
                storage_paths.append(row.get("storage_path"))
            else:
                storage_paths.append(row[1])

        # Best-effort storage cleanup before DB deletion.
